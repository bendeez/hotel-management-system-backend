...
class HotelDataSyncer:
    def __init__(self, task_count: int):
        self.task_count = task_count

    async def get_max_hotel_id(self):
    ...
    async def insert_entities_into_database(self, entities: list[dict], model: Any):
        tasks = []
        hotel_chunk = len(entities) / self.task_count
        for i in range(self.task_count):
            partitioned_entities = entities[
                int(i * hotel_chunk) : int((i + 1) * hotel_chunk)
            ]
            tasks.append(
                self.insert_partitioned_entities(
                    entities=partitioned_entities, model=model
                )
            )
        await asyncio.gather(*tasks)

    async def insert_hotels_into_database(self, df):
        hotels = []
        for index, hotel in df.iterrows():
            ...
            hotels.append(hotel)
        await self.insert_entities_into_database(entities=hotels, model=Hotels)

    async def sync_hotel_data_to_database(self, df):
        df = df.replace({np.nan: None}).sort_values(by=["title"])
        max_hotel_id = await self.get_max_hotel_id()
        await self.insert_hotels_into_database(df)
        async with SessionLocal() as db:
            id_filter = True
            if max_hotel_id:
            ...
            for index, hotel in df.iterrows():
                ...
                hotel_review = {
                    ...
                    "cleanliness_rating_out_of_10": hotel[
                        "cleanliness_rating_out_of_10"
                    ],
                ...
                hotel_house_rules = (
                    {
                        ...
                        "cancellation_payment": house_rules.get(
                        ...
                        "cards_accepted": house_rules.get(
            ...
            for model, entities in bulk_inserts_by_table.items():
                ...
                tasks.append(
                    self.insert_entities_into_database(entities=entities, model=model)
                )
            await asyncio.gather(*tasks)
