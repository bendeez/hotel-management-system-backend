...
df["rooms_to_price"] = (
    df["rooms_to_price"].apply(ht.safe_literal_eval).apply(ht.modify_room_to_price)
)
...
async def sync_to_database(df):
    async with SessionLocal() as db:
        for index, hotel in df.iterrows():
            ...
            rooms_to_price = (
                [ht.safe_literal_eval(room) for room in rooms_to_price]
                if rooms_to_price is not None
                else None
            )
            hotel_rooms = (
                [
                    Hotel_Rooms(
                        room_type=ht.safe_literal_eval(room.get("room_type")),
                        price=ht.safe_literal_eval(room.get("price")),
            ...
            house_rules = (
                ht.safe_literal_eval(hotel["house_rules"])
                if hotel["house_rules"] is not None
                else None
            )
            hotel_house_rules = (
                Hotel_House_Rules(
            ...
            guest_reviews = (
                [ht.safe_literal_eval(review) for review in guest_reviews]
            ...
            hotel_data = Hotels(
                ...
                amenities=ht.safe_literal_eval(hotel["amenities"]),
