...
class HotelWebCrawler:
    ...
    async def scrape_bulk_urls(self, context: BrowserContext, tabs_limit: int, urls: list[str]) -> list[str]:
        sem = asyncio.Semaphore(tabs_limit)
        tasks = []
        for url in urls:
            tasks.append(self.task_with_semaphore(sem=sem, task=self.scrape_page(context=context, url=url)))
    ...
    async def run(self):
        async with async_playwright() as playwright:
            ...
            google_suggestions_html = await self.scrape_page(context=context, url=google_suggestions_url)
            main_hotel_website_url = self.find_website_url(html=google_suggestions_html)
            main_hotel_website_html = await self.scrape_page(context=context, url=main_hotel_website_url)
            main_hotel_website_a_tag_urls = self.extract_urls_from_website(html=main_hotel_website_html)
            parsed_url = urlparse(main_hotel_website_url)
            base_url = f"{parsed_url.scheme}://{parsed_url.netloc}"
            main_hotel_website_absolute_paths = list(set([url for url in main_hotel_website_a_tag_urls if re.match("^/", url)]))
            urls_to_scrape = [f"{base_url}{path}" for path in main_hotel_website_absolute_paths]
            hotel_htmls = await self.scrape_bulk_urls(context=context, tabs_limit=5, urls=urls_to_scrape)
            hotel_texts = [self.extract_text_from_html(html=html) for html in hotel_htmls]
            await context.close()
