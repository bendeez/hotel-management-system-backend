...
def test_facility_creation(account, facility_service):
    _, account = account
    facility_info = FacilityCreate(
        title="spa", description="place to relax", account_id=account.id
    )
    facility = facility_service.create_facility(facility=facility_info)
    assert FacilityOut(id=1, **facility.__dict__) == FacilityOut(
        id=1, **facility_info.model_dump()
    )
