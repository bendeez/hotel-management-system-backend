...
async def user(user_service,password, auth_service) -> tuple[TokenCreate, Users]:
    user_account = await user_service.create_user_account(user=UserAccountCreate(email="user@gmail.com", password=password))
    tokens = await auth_service.verify_account(email=user_account.email, input_password=password)
    return tokens, user_account


@pytest.fixture(scope="session")
async def business(business_service, password, auth_service) -> tuple[TokenCreate, Business]:
    business_account = await business_service.create_business_account(business=BusinessAccountCreate(
        email="business@gmail.com",
        password=password,
        name="resort and fun",
        location="US"
    ))
    tokens = await auth_service.verify_account(email=business_account.email, input_password=password)
...
async def business_user(
    ...
    business_user_account = await business_service.create_business_user_account(business_user=
    ...
    tokens = await auth_service.verify_account(email=business_user_account.email, input_password=password)
...
async def make_http_request(client):
    async def _make_http_request(path, client=client,method=RequestMethod.GET,
                                 json=None,files=None,token=None):
        data = await http_request(client=client,path=path,method=method,
