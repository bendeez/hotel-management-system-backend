...
class HotelCleanDataTool:
    ...
    def modify_room_to_price(self, rooms):
        ...
        try:
            ...
            rooms_with_num_columns = [
                self.add_num_columns_to_room(room=room) for room in modified_rooms
            ]
    ...
    def add_new_num_guest_count_columns(self, room: dict):
        ...
        if guest_count:
            ...
            room["guest_count_numeric"] = guest_count
        return room

    def add_new_num_price_columns(self, room: dict):
        ...
        if price:
            ...
            if price_and_fee:
                ...
                if len(price_and_fee) == 2:
                    ...
                    room["tax_and_fee_numeric"] = tax_and_fee
                room["price_numeric"] = price_wo_fee
        return room

    def add_num_columns_to_room(self, room: dict):
        try:
            room = self.add_new_num_guest_count_columns(room=room)
            room = self.add_new_num_price_columns(room=room)
            return room
        except Exception as e:
            print(e)
            return None
